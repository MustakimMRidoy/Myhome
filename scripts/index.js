...
if (isAdsWindow) {
    ...
    function handleIframeLoad(iframe, windowId, defaultTitle) {
        ...
        function closeWindow(windowId) {
            if (!windows[windowId]) return;
		// ব্যবহারের উদাহরণ:
const adManager = new AdvertisementCleanupManager();

    // বিজ্ঞাপন পরিষ্কার
    await adManager.cleanupAds(windowElement);
    
    // উইন্ডো বন্ধের ইভেন্ট ট্রিগার
    windowElement.dispatchEvent(new Event('windowClosing'));
...
///-------------------------------------
// উন্নত বিজ্ঞাপন পরিষ্কারকরণ সিস্টেম
class AdvertisementCleanupManager {
    constructor() {
        // বিজ্ঞাপন সনাক্তকরণ প্যাটার্ন
        this.adPatterns = {
            classNames: ['ad-container', 'ad-wrapper', 'advertisement'],
            idPatterns: ['ad-', 'advertisement-', 'sponsor-'],
            attributePatterns: ['data-ad', 'data-advertisement']
        };
        
        // MutationObserver কনফিগারেশন
        this.observerConfig = {
            childList: true,
            subtree: true,
            attributes: true
    ...
    /**
     * উইন্ডো এলিমেন্ট থেকে সকল বিজ্ঞাপন মুছে ফেলা
     * @param {HTMLElement} windowElement - টার্গেট উইন্ডো এলিমেন্ট
     * @returns {Promise} ক্লিনআপ অপারেশন সম্পন্ন হওয়ার প্রমিস
     */
    async cleanupAds(windowElement) {
        try {
            // ১. সকল iframe এর কার্যক্রম বন্ধ করা
            await this.stopIframeOperations(windowElement);
            
            // ২. ডাইনামিক বিজ্ঞাপন এলিমেন্ট ট্র্যাক করা
            this.setupDynamicAdTracking(windowElement);
            
            // ৩. বিদ্যমান বিজ্ঞাপন অপসারণ
            this.removeExistingAds(windowElement);
            
            // ৪. ইনজেকশন প্রতিরোধ সিস্টেম সেটআপ
            this.preventAdInjection(windowElement);
            
            return true;
        } catch (error) {
            console.error('বিজ্ঞাপন পরিষ্কারকরণে সমস্যা:', error);
            return false;
        }
    }

    /**
     * iframe এর সকল কার্যক্রম বন্ধ করা
     * @param {HTMLElement} element
     */
    async stopIframeOperations(element) {
        const iframes = element.querySelectorAll('iframe');
        
        for (const iframe of iframes) {
            try {
                // iframe এর কন্টেন্ট স্ক্রিপ্ট বন্ধ করা
                if (iframe.contentWindow) {
                    iframe.contentWindow.stop();
                }
                
                // সকল ইভেন্ট লিসেনার রিমুভ
                const clone = iframe.cloneNode(false);
                if (iframe.parentNode) {
                    iframe.parentNode.replaceChild(clone, iframe);
                }
                
                // iframe সোর্স খালি করা
                clone.src = 'about:blank';
                await new Promise(resolve => setTimeout(resolve, 50));
                clone.remove();
            } catch (e) {
                console.warn('iframe বন্ধ করতে সমস্যা:', e);
            }
        }
    }

    /**
     * ডাইনামিক বিজ্ঞাপন ট্র্যাকিং
     * @param {HTMLElement} element
     */
    setupDynamicAdTracking(element) {
        const observer = new MutationObserver((mutations) => {
            for (const mutation of mutations) {
                if (mutation.addedNodes.length) {
                    this.checkAndRemoveAds(mutation.addedNodes);
                }
            }
        });
        ...
        // উইন্ডো বন্ধের সময় অবজার্ভার ডিসকানেক্ট
        element.addEventListener('windowClosing', () => {
            observer.disconnect();
        }, { once: true });
    }

    /**
     * বিদ্যমান বিজ্ঞাপন অপসারণ
     * @param {HTMLElement} element
     */
    removeExistingAds(element) {
        // ক্লাস দ্বারা খোঁজা
        this.adPatterns.classNames.forEach(className => {
            element.querySelectorAll(`.${className}`).forEach(ad => this.safeRemove(ad));
        });

        // ID দ্বারা খোঁজা
        this.adPatterns.idPatterns.forEach(pattern => {
            element.querySelectorAll(`[id*="${pattern}"]`).forEach(ad => this.safeRemove(ad));
        });

        // অ্যাট্রিবিউট দ্বারা খোঁজা
        this.adPatterns.attributePatterns.forEach(pattern => {
            element.querySelectorAll(`[${pattern}]`).forEach(ad => this.safeRemove(ad));
        });
    }

    /**
     * নিরাপদে এলিমেন্ট রিমুভ
     * @param {HTMLElement} element
     */
    safeRemove(element) {
        try {
            // এলিমেন্টের সকল ইভেন্ট লিসেনার রিমুভ
            ...
            console.warn('এলিমেন্ট রিমুভ করতে সমস্যা:', e);
        }
    }

    /**
     * বিজ্ঞাপন ইনজেকশন প্রতিরোধ
     * @param {HTMLElement} element
     */
    preventAdInjection(element) {
        // DOM মডিফিকেশন প্রতিরোধ
        const preventModification = (e) => {
            if (this.isAdElement(e.target)) {
                e.preventDefault();
            }
        };

        element.addEventListener('DOMNodeInserted', preventModification, true);
        element.addEventListener('DOMNodeInsertedIntoDocument', preventModification, true);
        
        // উইন্ডো বন্ধের সময় ইভেন্ট লিসেনার রিমুভ
        element.addEventListener('windowClosing', () => {
            element.removeEventListener('DOMNodeInserted', preventModification, true);
            element.removeEventListener('DOMNodeInsertedIntoDocument', preventModification, true);
        }, { once: true });
    }

    /**
     * বিজ্ঞাপন এলিমেন্ট চেক
     * @param {HTMLElement} element
     * @returns {boolean}
     */
    isAdElement(element) {
        ...
        // ক্লাস চেক
        const hasAdClass = this.adPatterns.classNames.some(pattern => 
            element.classList.contains(pattern));
        if (hasAdClass) return true;

        // ID চেক
        const hasAdId = this.adPatterns.idPatterns.some(pattern => 
            element.id && element.id.includes(pattern));
        if (hasAdId) return true;

        // অ্যাট্রিবিউট চেক
        return this.adPatterns.attributePatterns.some(pattern => 
